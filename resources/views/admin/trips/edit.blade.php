            ...
            <form method="POST" action="{{ route('admin.trips.update', $trip) }}">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="hidden" name="vehicle_id" id="vehicle_id" value="{{ old('vehicle_id', $selectedVehicleId) }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="start_city_select" class="form-select" required>
                            ...
                            @foreach($cities as $city)
                                <option value="{{ $city->id }}" {{ old('start_city_id', optional($trip->stops->first()?->stop)->city_id) == $city->id ? 'selected' : '' }}>{{ $city->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select id="end_city_select" class="form-select" required>
                            ...
                            @foreach($cities as $city)
                                <option value="{{ $city->id }}" {{ old('end_city_id', optional($trip->stops->last()?->stop)->city_id) == $city->id ? 'selected' : '' }}>{{ $city->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" id="total_seats" name="total_seats" value="{{ old('total_seats', $trip->total_seats) }}" class="form-control @error('total_seats') is-invalid @enderror" min="1" required>
                        @error('total_seats')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Vehicle Capacity</label>
                        <input type="text" id="vehicle_capacity_display" class="form-control" readonly value="{{ old('vehicle_capacity', $selectedVehicleSummary ?: 'No approved vehicle') }}">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Available Seats</label>
                        <input type="number" id="available_seats" name="available_seats" value="{{ old('available_seats', $trip->available_seats) }}" class="form-control @error('available_seats') is-invalid @enderror" min="1" required>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const initialFromStopId = @json(old('from_stop_id', $fromStopId));
    ...
    if (startCitySelect?.value) {
        renderStopsForCity(fromStopSelect, startCitySelect.value, initialFromStopId);
    }

    if (endCitySelect?.value) {
        renderStopsForCity(toStopSelect, endCitySelect.value, initialToStopId);
    }

    if (Array.isArray(initialSelectedStopIds)) {
        initialSelectedStopIds.forEach(stopId => {
            const city = citiesData.find(item => item.stops.some(stop => String(stop.id) === String(stopId)));
            const stop = city?.stops.find(item => String(item.id) === String(stopId));
            if (stop) {
                selectedStops.push(stop);
            }
